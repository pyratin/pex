...
const gridStrokeDefinition = /** @type {pixiJs.StrokeStyle} */ ({
  width: 1,
  color: 0xcccccc
});
...
const Control = ({ transformOrigin, maxCollection, transformOriginSet }) => {
  const definitionCollection = ['x', 'y'].map((key, index) => ({
    key,
    value: transformOrigin[index],
    max: maxCollection[index]
  }));
  ...
  useEffect(() => {
    rootRef.current.render(
      <Control
        ...
        maxCollection={Object.values(dimension)}
        transformOriginSet={transformOriginSet}
      />
    );
  }, [transformOrigin, transformOriginSet]);
